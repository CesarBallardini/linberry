...
def find_berry(userdev=None, userbus=None, verbose=True):
	...
	bb_messenging.status("MSG:Escaneando Dispositivos RIM:")
	berry=None
	if userdev and userbus:
		...
		for bus in usb.busses():
			for dev in bus.devices:
				if(verbose):
					bb_messenging.log("Bus %s Dispositivo %s: ID %04x:%04x" % (bus.dirname,dev.filename,dev.idVendor,dev.idProduct))
	...
	if verbose :
		bb_messenging.log("MSG:Escaneado finalizado")
...
def read_bb_endpoints(device, userInterface):
	...
	if config.has_section(SECTION_SCANNED_EP):
		...
		bb_messenging.log("MSG:Puntos de entrada para este Dispositivo: "+str(device.interface)+", "+str(device.readpt)+", "+str(device.writept)+", "+str(device.modem_readpt)+", "+str(device.modem_writept))
		# return saved data if good (earlier version saved bad ones)
		if device.readpt != -1 and device.writept !=-1:
			return device
		else:
			print "MSG:Puntos de entrada invalidos reescaneando."
