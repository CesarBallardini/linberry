...
class BBModem:		
	...
	def init(self):
		...
		try:
			...
			bb_messenging.log("ERR:Falla en el colgado: " + str(error))
		...
		while True:
			...
			try:
				...
				bb_messenging.log("ERR:Error de lectura: " + error)
				answer = ""
			if len(answer) == 0:
				reset_time += 5
				if not resetted:
					bb_messenging.status("MSG:BlackBerry Modem no responde ... Intentando nuevamente (espere)")
				...
				if reset_time > 30:
					...
					#]
					bb_messenging.warn("ERR:Tiempo de conn agotado")
					bb_messenging.log("***********************************************")
					os._exit(0)
	...
	def start(self, pppConfig, pppdCommand):
		...
		bb_messenging.status("MSG:Inicializando BlackBerry Modem")
		try:
			self.init()
		except:
		...
		bb_messenging.status("MSG:BlackBerry Modem Iniciado")
		...
		bb_messenging.log("********************************************")
		bb_messenging.status("MSG:BlackBerry Modem Listo " + os.ttyname(self.slave))
		bb_messenging.log("  ^C Para Finalizar")
		bb_messenging.log("********************************************")
	...
	def do_shutdown(self):
		...
		bb_messenging.log("MSG:Apagando Interface")
		...
		try:
			...
			bb_messenging.status("MSG:Desconectando BlackBerry Modem")
			self.bbThread.stop()
		...
		# done
		self.shutting_down = False
		bb_messenging.status("MSG:BlackBerry Modem Desconectado")
...
# Increased from .5 to 2 as had report .5 was not always enough
		...
		if len(answer) > 8 and answer[0] == 0x4:
			...
			bb_messenging.log("Respuesta inesperada: "+str(answer))
		bb_messenging.warn(["ERR:ERROR DE CLAVE"])
		os._exit(0)
...
class BBModemThread(threading.Thread):	
	...
	def run (self):
		self.done = False
		bb_messenging.status("MSG:Iniciando BlacBerry Modem")
		while(not self.done):
			try:
				try:
				...
				if(self.done):
					...
					raise
		bb_messenging.status("MSG:BlackBerry Modem finalizado")
