...
Usage() {
	echo "Main Linberry build test script."
	echo
	echo "Usage:"
	echo "       ./buildtest.sh /path/to/libopensync-0.22.tar.bz2"
	echo
	echo "Or, write a ~/.linberrytest2 file that contains shell commands"
...
#
# Do we have a ~/.linberrytest2 config?
#
if [ -f ~/.linberrytest2 ] ; then
	. ~/.linberrytest2
fi
...
#
# Move .. linberry into its own buildable directory
#

mkdir -p build/linberry
(tar -C .. --exclude=CVS --exclude=.git --exclude=test/build -cf - . | \
	tar -C build/linberry -xf -)
diff -ruN --exclude=CVS --exclude=.git --exclude=test --exclude=build .. build/linberry



#
# Prepare for Linberry building
#
cd build/linberry
