...
if [ "$1" = "cleanall" ] ; then
	...
	rm -rf debian/linberry
	rm -f build-arch-stamp build-indep-stamp configure-stamp \
		debian/linberry.substvars debian/files \
		tools/bcharge
	...
	ctags -R -f ~/tags-linberry --tag-relative=yes
	...
	if [ "$2" = "0.22" ] ; then
		...
		if [ -d $OS_DIR ] ; then
			...
			(cd $OS_DIR && ctags -R -a -f ~/tags-linberry --tag-relative=yes)
		fi
	fi
