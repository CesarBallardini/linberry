...
void SerialDataCallback(void *context, const unsigned char *data, int len)
{
	...
	while( len ) {
		...
		if( written > 0 ) {
			...
			linberryverbose("Error in write()");
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	try {
		...
		if( logfile.size() ) {
			...
			Linberry::Init(data_dump, log.get());
		}
		else {
			Linberry::Init(data_dump, &std::cerr);
		}
		...
		if( !force_serial && device.HasIpModem() ) {
			linberryverbose("Using IpModem mode...");
			...
			if( force_serial ) {
				linberryverbose("Using Serial mode per command line...");
			}
			else {
				linberryverbose("No IpModem mode available, using Serial mode...");
			}
