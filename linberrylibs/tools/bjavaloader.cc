...
void SetTime(Linberry::Mode::JavaLoader *javaloader, const char *timestr)
{
	time_t when;
...
void SendAppFile(Linberry::Mode::JavaLoader *javaloader, const char *filename)
{
	ifstream file(filename);
	javaloader->LoadApp(file);
}

void GetScreenshot(Linberry::Mode::JavaLoader *javaloader, const char *filename)
{
	...
	if (fp == NULL) {
		throw runtime_error(string("imposible abrir: ") + filename);
	}
	AutoClose ac(fp);
...
void SaveModule(Linberry::Mode::JavaLoader *javaloader, const char *filename)
{
...
// 05 de Junio de 2012 Israel Marrero
...
if (argc ==1){
	cerr
            << "Ejecutado solo desde LinBerry, saliendo...." 
	    <<   endl;
exit(1);
}

//si el parametro es mi ci chevere si no se sale
if( strcmp(argv[argc-1],"MICLAVE")!=0){
	cerr
           << "Ejecutado solo desde LinBerry, saliendo...." 
	   << endl;
...
//******************************************************************
	...
	try {
		...
		if( argc < 1 ) {
			cerr << "no entiendo el comando o faltan parametros" << endl;
			Usage();
			return 1;
		}
		...
		argc --;
		argv ++;
// se agrego esta linea para evitar que tome como parte del string la clave MICLAVE
		argc --;
// fin de lo agregado
		...
		Linberry::Mode::JavaLoader javaloader(con);
