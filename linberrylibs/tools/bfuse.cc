///
/// \file	bfuse.cc
///		FUSE filesystem for Blackberry databases, using Linberry.
///
...
typedef std::auto_ptr<Linberry::Parser>		ParserPtr;
...
{
public:
	Linberry::Mode::Desktop &m_desk;
	std::string m_name;
	const Linberry::DatabaseItem *m_pdb;

public:
	Database(Linberry::Mode::Desktop &desktop,
		const std::string &pin, const Linberry::DatabaseItem *pdb)
		: m_desk(desktop)
		, m_pdb(pdb)
	{
	...
	virtual int ReadDir(void *buf, fuse_fill_dir_t filler)
	{
		...
		Linberry::RecordStateTable rst;
		m_desk.GetRecordStateTable(m_pdb->Number, rst);

		Linberry::RecordStateTable::StateMapType::iterator
...
public:
	...
	DesktopCon(const Linberry::ProbeResult &result, const std::string &pin)
		: m_con(result)
		, m_desk(m_con)
		, m_pin(pin)
	{
...
{
public:
	typedef std::auto_ptr<Linberry::Probe>			ProbePtr;
...
public:
		...
		, m_password(password)
	{
		...
		m_error_log = "Hello FUSE world.  This is Linberry.  Pleased to meet you.\n";
	}
