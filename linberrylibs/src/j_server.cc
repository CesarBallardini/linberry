...
JDWServer::JDWServer(Linberry::Mode::JVMDebug &device,
...
bool JDWServer::Start() 
{
	...
	if (sockfd < 0) {
		throw Linberry::ErrnoError("JDWServer::Start: Cannot open socket.", errno);
	}
	...
	if (rc < 0) {
		...
		throw Linberry::ErrnoError("JDWServer::Start: Cannot bind socket", code);
	}
	...
	if (listen(sockfd, SOMAXCONN) < 0) {
		...
		throw Linberry::ErrnoError("JDWServer::Start: Cannot listen on socket", code);
	}
			...
			if (value < 0) {
				...
				while (!ret) {
					...
					if (command.GetSize() > 0) {
						// Convert to packet
						rpack = (const Linberry::Protocol::JDWP::Packet *) command.GetData();
...
void JDWServer::CommandVersion(Data &cmd)
{
	...
	Linberry::Protocol::JDWP::Packet &packet = *cpack;
...
void JDWServer::CommandIdSizes(Data &cmd)
{
	...
	Linberry::Protocol::JDWP::Packet packet;
