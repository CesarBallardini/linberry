...
Error::Error(int errcode, const std::string &str)
	: Linberry::Error(GetErrorString(errcode, str))
	, m_errcode(errcode)
{
}
...
bool JDWP::Read(int socket, Linberry::Data &data, int timeout)
{
	int ret;
...
bool JDWP::Write(int socket, const Linberry::Data &data, int timeout)
{
	int ret;
