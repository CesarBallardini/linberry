...
void SocketZero::AppendFragment(Data &whole, const Data &fragment)
{
	...
	Linberry::Protocol::Packet *wpack = (Linberry::Protocol::Packet *) whole.GetBuffer();
...
unsigned int SocketZero::MakeNextFragment(const Data &whole, Data &fragment, unsigned int offset)
{
	...
	Linberry::Protocol::Packet *wpack = (Linberry::Protocol::Packet *) buf;
...
{
	// build open command
	Linberry::Protocol::Packet packet;
	packet.socket = 0;
...
void SocketZero::Send(Linberry::Packet &packet, int timeout)
{
...
SocketHandle SocketZero::Open(uint16_t socket, const char *password)
{
	...
	if( m_halfOpen ) {
		...
		// LINBERRY_MIN_PASSWORD_TRIES or more tries remaining...
		// we want to give the user at least some chance on a
		// Windows machine before the device commits suicide.
		if( m_remainingTries < LINBERRY_MIN_PASSWORD_TRIES ) {
			throw BadPassword("Fewer than " LINBERRY_MIN_PASSWORD_TRIES_ASC " password tries remaining in device. Refusing to proceed, to avoid device zapping itself.  Use a Windows client, or re-cradle the device.",
				m_remainingTries,
				true);
		}
...
void Socket::Send(Linberry::Packet &packet, int timeout)
{
	Send(packet.m_send, packet.m_receive, timeout);
}
...
void Socket::Packet(Linberry::Packet &packet, int timeout)
{
	Packet(packet.m_send, packet.m_receive, timeout);
}

void Socket::Packet(Linberry::JLPacket &packet, int timeout)
{
	if( packet.HasData() ) {
		HideSequencePacket(false);
...
void Socket::Packet(Linberry::JVMPacket &packet, int timeout)
{
	HideSequencePacket(false);
