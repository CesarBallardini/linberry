...
Error::Error(const std::string &str)
	: Linberry::Error(GetErrorString(0, str))
	, m_libusb_errcode(0)
{
}

Error::Error(int libusb_errcode, const std::string &str)
	: Linberry::Error(GetErrorString(libusb_errcode, str))
...
bool Device::BulkRead(int ep, Linberry::Data &data, int timeout)
{
	int ret;
	do {
		data.QuickZap();
...
bool Device::BulkWrite(int ep, const Linberry::Data &data, int timeout)
{
...
bool Device::InterruptRead(int ep, Linberry::Data &data, int timeout)
{
	int ret;
	do {
		data.QuickZap();
...
bool Device::InterruptWrite(int ep, const Linberry::Data &data, int timeout)
{
...
bool EndpointDiscovery::Discover(struct usb_interface_descriptor *interface, int epcount)
{
	...
	if( interface->extra ) {
		...
		Linberry::Data data(interface->extra, interface->extralen);
		dout(data);
	}
...
bool ConfigDiscovery::Discover(Usb::DeviceIDType devid, int cfgcount)
{
	...
	for( int i = 0; i < cfgcount; i++ ) {
		...
		if( desc.desc.extra ) {
			...
			Linberry::Data data(desc.desc.extra, desc.desc.extralen);
			dout(data);
		}
