			     ...
			     uint8_t alpha)
{
	if( depth != 24 && depth != 32 )
		throw Linberry::Error("ScreenshotToRGB: depth must be 24 or 32");
	...
	if( data_size != 2 && data_size != 4 )
		throw Error("ScreenshotToRGB: Screenshot depth is not supported (Linberry supports 2 byte or 4 byte pixels in device screenshots)");
	...
	for( size_t j = 0; j < height; j++ ) {
		for( size_t i = 0; i < width; i++ ) {
			...
			default:
				...
				throw Error("Error extraterrestre");
			}
