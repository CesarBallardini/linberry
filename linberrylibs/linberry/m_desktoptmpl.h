...
#ifndef __LINBERRY_M_DESKTOPTMPL_H__
#define __LINBERRY_M_DESKTOPTMPL_H__
...
void Desktop::LoadDatabaseByType(StorageT &store)
{
	...
	Linberry::RecordParser<RecordT, StorageT> parser(store);
...
{
	unsigned int dbId = this->GetDBID( RecordT::GetDBName() );
	Linberry::RecordBuilder<RecordT, StorageT> build(store);
...
void Desktop::AddRecordByType(uint32_t recordId, const RecordT &rec)
{
	...
	Linberry::RecordFetch<RecordT> fetch(r);
	Linberry::RecordBuilder<RecordT, Linberry::RecordFetch<RecordT> > build(fetch);
