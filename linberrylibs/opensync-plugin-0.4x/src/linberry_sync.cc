...
#include <linberry/dll.h>
#include "linberry_sync.h"
...
typedef Linberry::vSmartPtr<OSyncList, OSyncList, &osync_list_free> AutoOSyncList;
		...
		osync_bool slow_sync)
{
	...
	using Linberry::RecordStateTable;
...
bool FinishSync(OSyncContext *ctx, LinberryEnvironment *env, DatabaseSyncState *pSync)
{
	...
	Linberry::Mode::Desktop &desktop = *env->m_pDesktop;
...
static bool linberry_contact_initialize(LinberryEnvironment *env, OSyncPluginInfo *info, OSyncError **error)
{
	...
	for(r = objformatsinks.Get();r;r = r->next) {
		...
		if(!strcmp("vcard30", osync_objformat_sink_get_objformat(objformatsink))) {
			trace.log("vcard30 found in linberry-sync");
			hasObjFormat = true;
			break;
		}
	}
...
static bool linberry_calendar_initialize(LinberryEnvironment *env, OSyncPluginInfo *info, OSyncError **error)
{
...
static bool linberry_journal_initialize(LinberryEnvironment *env, OSyncPluginInfo *info, OSyncError **error)
{
...
static bool linberry_todo_initialize(LinberryEnvironment *env, OSyncPluginInfo *info, OSyncError **error)
{
...
static void *initialize(OSyncPlugin *plugin, OSyncPluginInfo *info, OSyncError **error)
{
	...
	LinberryEnvironment *env = 0;

	try {
		env = new LinberryEnvironment(info);
		...
		if (linberry_calendar_initialize(env, info, error)) {
		...
		if (linberry_contact_initialize(env, info, error)) {
		...
		if (linberry_journal_initialize(env, info, error)) {
		...
		if (linberry_todo_initialize(env, info, error)) {
...
static osync_bool discover(OSyncPluginInfo *info, void *userdata, OSyncError **error)
{
	...
	osync_version_set_plugin(version, "Linberry");
...
static void connect(OSyncObjTypeSink *sink, OSyncPluginInfo *info, OSyncContext *ctx, void *userdata)
{
	...
	try {
		LinberryEnvironment *env = (LinberryEnvironment *) userdata;
...
static void commit_change(OSyncObjTypeSink *sink, OSyncPluginInfo *info, OSyncContext *ctx, OSyncChange *change, void *userdata)
{
	...
	try {
		...
		if( !pSync->m_Sync ) {
			osync_context_report_error(ctx, OSYNC_ERROR_GENERIC,
				"This object type is disabled in the linberry-sync config");
			return;
		}

		// make references instead of pointers
		Linberry::RecordStateTable &table = pSync->m_Table;
		Linberry::Mode::Desktop &desktop = *env->m_pDesktop;
		unsigned int dbId = pSync->m_dbId;


		// either generate or retrieve the record ID, based on type
		Linberry::RecordStateTable::IndexType StateIndex;
...
static void finalize(void *data)
{
	...
	LinberryEnvironment *env = (LinberryEnvironment *) data;
...
osync_bool get_sync_info(OSyncPluginEnv *env, OSyncError **error)
{
	...
	// Describe our plugin
	osync_plugin_set_name(plugin, "linberry-sync");
	osync_plugin_set_longname(plugin, "Linberry OpenSync plugin v0.16 for the Blackberry handheld");
