...
void vCard::AddAddress(const char *rfc_type, const Linberry::PostalAddress &address)
{
	// add label first
...
void vCard::AddCategories(const Linberry::CategoryList &categories)
{
	if( !categories.size() )
		return;

	vAttrPtr cat = NewAttr("CATEGORIES");		// RFC 2426, 3.6.1
	Linberry::CategoryList::const_iterator i = categories.begin();
...
void vCard::ParseAddress(vAttr &adr, Linberry::PostalAddress &address)
{
	// RFC 2426, 3.2.1
...
void vCard::ParseCategories(vAttr &cat, Linberry::CategoryList &cats)
{
	int i = 0;
...
// Main conversion routine for converting from Linberry::Contact to
// a vCard string of data.
const std::string& vCard::ToVCard(const Linberry::Contact &con)
{
	Trace trace("vCard::ToVCard");
	std::ostringstream oss;
	con.Dump(oss);
	trace.logf("ToVCard, initial Linberry record: %s", oss.str().c_str());
	...
	m_LinberryContact = con;

	//
	// begin building vCard data
	//

	AddAttr(NewAttr("PRODID", "-//OpenSync//NONSGML Linberry Contact Record//EN"));
	...
	Linberry::Contact::EmailList::const_iterator eai = con.EmailAddresses.begin();
...
// to a Linberry::Contact object.
const Linberry::Contact& vCard::ToLinberry(const char *vcard, uint32_t RecordId)
{
	using namespace std;

	Trace trace("vCard::ToLinberry");
	trace.logf("ToLinberry, working on vcard data: %s", vcard);
	...
	Linberry::Contact &con = m_LinberryContact;
	con.SetIds(Linberry::Contact::GetDefaultRecType(), RecordId);
	...
	for( int i = 0; tel.Get(); tel = GetAttrObj("TEL", ++i) )
	{
		...
		if( fax_type && (mobile_type || pager_type) ) {
			...
			trace.logf("ToLinberry: skipping phone number due to TYPE conflict: fax cannot coexist with %s: %s",
		...
		// bbs/video/modem   entire TEL ignored by Linberry
		// isdn              entire TEL ignored by Linberry
...
bool VCardConverter::ParseData(const char *data)
{
	...
	try {
		...
		vCard vcard;
		m_Contact = vcard.ToLinberry(data, m_RecordId);
...
void VCardConverter::operator()(const Linberry::Contact &rec)
{
...
bool VCardConverter::operator()(Linberry::Contact &rec, unsigned int dbId)
{
...
char* VCardConverter::GetRecordData(LinberryEnvironment *env, unsigned int dbId,
...
bool VCardConverter::CommitRecordData(LinberryEnvironment *env, unsigned int dbId,
	...
	const char *data, bool add, std::string &errmsg)
{
	...
	Linberry::RecordBuilder<Linberry::Contact, VCardConverter> builder(convert);
