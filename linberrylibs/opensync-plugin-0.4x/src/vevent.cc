...
void vCalendar::RecurToVCal()
{
	...
	using namespace std;
	Linberry::Calendar &cal = m_LinberryCal;
	...
	default:
		throw ConvertError("Unknown RecurringType in Linberry Calendar object");
	}
...
void vCalendar::RecurToLinberryCal(vAttr& rrule, time_t starttime)
{
	using namespace Linberry;
	using namespace std;
	Linberry::Calendar &cal = m_LinberryCal;
	Trace trace("vCalendar::RecurToLinberryCal");
	...
	do {
		...
		args[n]=v;
		trace.logf("RecurToLinberryCal: |%s|%s|",n.c_str(),v.c_str());
	} while(1);
	...
	
	{
		...
		if( te.IsSet() ) {
			trace.log("RecurToLinberryCal: Cannot find local timezone diff... assuming 0");
			zoneoffset = 0;
		}
	}
	...
	if(args.find(string("FREQ"))==args.end()) {
		trace.logf("RecurToLinberryCal: No frequency specified!");
		return;
	}
	...
	if(args["FREQ"]==string("DAILY")) {
		...
		if(args.find(string("BYDAY"))!=args.end()) {
			...
			// handle error here
			trace.logf("RecurToLinberryCal: no BYDAY on weekly event");
		}
		if(count) {
		...
		if(args.find(string("BYMONTHDAY"))!=args.end()) {
			...
			if(args.find(string("BYDAY"))!=args.end()) {
				...
				trace.logf("RecurToLinberryCal: No qualifier on MONTHLY freq");
			}
		}
		if(count) {
		...
		if(args.find(string("BYMONTH"))!=args.end()) {
			...
			if(args.find(string("BYMONTHDAY"))!=args.end()) {
				...
				if(args.find(string("BYDAY"))!=args.end()) {
					...
					trace.logf("RecurToLinberryCal: No qualifier on YEARLY freq");
				}
			}
		} else {
...
// Main conversion routine for converting from Linberry::Calendar to
// a vCalendar string of data.
const std::string& vCalendar::ToVCal(const Linberry::Calendar &cal)
{
	Trace trace("vCalendar::ToVCal");
	std::ostringstream oss;
	cal.Dump(oss);
	trace.logf("ToVCal, initial Linberry record: %s", oss.str().c_str());
	...
	AddAttr(NewAttr("PRODID", "-//OpenSync//NONSGML Linberry Calendar Record//EN"));
...
// to a Linberry::Calendar object.
const Linberry::Calendar& vCalendar::ToLinberry(const char *vcal, uint32_t RecordId)
{
	using namespace std;

	Trace trace("vCalendar::ToLinberry");
	trace.logf("ToLinberry, working on vcal data: %s", vcal);
	...
	int zoneoffset;
	{
		...
		zoneoffset = osync_time_timezone_diff(localtime(&now), te);
		if( te.IsSet() ) {
			trace.log("ToLinberry: Cannot find local timezone diff... assuming 0");
			zoneoffset = 0;
		}
	}

	Linberry::Calendar &rec = m_LinberryCal;
	rec.SetIds(Linberry::Calendar::GetDefaultRecType(), RecordId);
	...
	if( !end.size() ) {
		...
		TossError te("ToLinberry, EndTime 2 unix", trace);
	...
	if(rrule.Get()) {
		RecurToLinberryCal(rrule, rec.StartTime);
	}
	...
	if( trigger_obj.Get() ) {
		...
		if( trigger.size() == 0 ) {
			...
			TossError te("ToLinberry, NotificationTime 2 unix", trace);
...
bool VEventConverter::ParseData(const char *data)
{
	...
	try {
		...
		vCalendar vcal;
		m_Cal = vcal.ToLinberry(data, m_RecordId);
...
void VEventConverter::operator()(const Linberry::Calendar &rec)
{
...
bool VEventConverter::operator()(Linberry::Calendar &rec, unsigned int dbId)
{
...
char* VEventConverter::GetRecordData(LinberryEnvironment *env, unsigned int dbId,
...
bool VEventConverter::CommitRecordData(LinberryEnvironment *env, unsigned int dbId,
	...
	const char *data, bool add, std::string &errmsg)
{
	...
	Linberry::RecordBuilder<Linberry::Calendar, VEventConverter> builder(convert);
