...
backup tool that comes with Linberry, or the RIM Windows Desktop software,
...
in the Linberry Opensync 0.22 plugin.</p>
...
<p>For example, using msynctool, you can create a group called EvoLinberry
which will sync your Blackberry and Evolution like this:</p>

<pre>
	msynctool --delgroup EvoLinberry
	msynctool --addgroup EvoLinberry
	msynctool --addmember EvoLinberry evo2-sync
	msynctool --addmember EvoLinberry linberry-sync
	msynctool --configure EvoLinberry 1
	msynctool --configure EvoLinberry 2
	msynctool --showgroup EvoLinberry
</pre>
...
<p>Linberry's plugin member takes a simple text file.  The default configuration
that comes with Linberry is shown below:</p>

<pre>
	#
	# This is the default configuration file for the linberry-sync opensync plugin.
...
for Linberry and OpenSync.  The debug packages for Linberry are available
at the
...
keyword in the Linberry plugin configuration above, and send the last
