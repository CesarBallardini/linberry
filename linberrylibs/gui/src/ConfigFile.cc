...
/// passed a bad pin.
ConfigFile::ConfigFile(Pin pin,
		       const Linberry::DatabaseDatabase &db)
	: m_pin(pin)
	, m_loaded(false)
	, m_promptBackupLabel(false)
{
...
void ConfigFile::BuildFilename()
{
	...
	//m_filename += "/.linberry/backup/";
	m_filename += "/linberrybackup/";
	m_filename += m_pin.str();
	m_filename += "/config.txt";
}
...
void ConfigFile::BuildDefaultPath()
{
	struct passwd *pw = getpwuid(getuid());
	m_path = pw->pw_dir;
	//m_path += "/.linberry/backup/";
	m_path += "/linberrybackup/";
	m_path += m_pin.str();
}
...
//*********************************************************
// Israel Marrero
			else if( keyword == "restore_filename" ) {
				iss >> std::ws;
				std::getline(iss,m_restore_filename );
			}
//***************************************
		}
	}
...
bool ConfigFile::Save()
{
	...
	//if( m_restore_filename.size() ) {
	//	out << "restore_filename " << m_restore_filename << std::endl;
	//}
...
/// defaults.
void ConfigFile::Enlighten(const Linberry::DatabaseDatabase &db)
{
	if( !m_loaded ) {
		...
		Linberry::DatabaseDatabase::DatabaseArrayType::const_iterator i =
			db.Databases.begin();
		for( ; i != db.Databases.end(); ++i ) {
			...
			if( i->Name != Linberry::Message::GetDBName() &&
