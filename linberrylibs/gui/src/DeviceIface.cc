...
void DeviceInterface::BackupThread()
{
	...
	try {
		...
		for( ; name != m_dbList.end(); ++name ) {
			...
			//std::cerr << "Respaldando Basededatos: " << *name << std::endl;

			//añadido Israel Marrero para evitar que la base salga muy rapido

			 clock_t start_time, cur_time;
			 start_time = clock();
			 while((clock() - start_time) < 0.3 * CLOCKS_PER_SEC)
			 {
			 }
	

			std::cout << "Respaldando Basededatos: " << *name << "\n";
			//fin añadido
...
bool DeviceInterface::Connect()
{
	try {
		Disconnect();
		m_con = new Linberry::Controller(m_dev->result);
		m_desktop = new Linberry::Mode::Desktop(*m_con);
		m_desktop->Open();
		return true;
	}
	catch( Linberry::BadPassword & ) {
		// pass on to the caller
		throw;
	}
	catch( Linberry::BadSize & ) {
		// pass on to the caller
		Disconnect();
		throw;
	}
	catch( Linberry::Error &e ) {
		Disconnect();
		return False(e.what());
	}
}
...
unsigned int DeviceInterface::GetRecordTotal(const ConfigFile::DBListType &backupList) const
{
	...
	Linberry::DatabaseDatabase::DatabaseArrayType::const_iterator
...
void DeviceInterface::ParseHeader(const Linberry::Data &data, size_t &offset)
{
}

void DeviceInterface::ParseFields(const Linberry::Data &data,
				  size_t &offset,
...
void DeviceInterface::BuildHeader(Linberry::Data &data, size_t &offset)
{
	// nothing to do
}

void DeviceInterface::BuildFields(Linberry::Data &data, size_t &offset,
				  const Linberry::IConverter *ic)
{
...
void DeviceInterface::SkipCurrentDB() throw()
{
	...
	try {
		...
		std::cerr << "EXCEPTION IN SkipCurrentDB()!  "
			"Please report to Linberry mailing list." << std::endl;
	}
}
...
Device::Device(Linberry::ProbeResult pr) : result(pr)
{
}
