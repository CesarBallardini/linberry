...
cp "$SPECPATH" "$CHROOT/$TARGET/usr/src/redhat/SPECS/linberry.spec"
chroot "$CHROOT/$TARGET" /bin/sh -lc "rm -f /usr/src/redhat/RPMS/i386/* /usr/src/redhat/SRPMS/* && cd /usr/src/redhat/SPECS && rpmbuild --target i386 -ba linberry.spec --with gui --with opensync && cd /usr/src/redhat/RPMS/i386"
