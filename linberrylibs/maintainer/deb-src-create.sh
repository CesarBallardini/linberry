...
#
# The following dance is to keep the .orig.tar.gz containing linberry-0.12/
# as the directory, and not something like linberry-0.12.orig/ which gets
# corrected by Debian tools.  If there is a better way, please send a patch.
cp -a $DIRNAME $DIRNAME.patched
rm -rf $DIRNAME/debian
tar -cf - $DIRNAME | gzip -9c > linberry_$MAJOR.$MINOR.orig.tar.gz
rm -rf $DIRNAME
mv $DIRNAME.patched $DIRNAME
dpkg-source -b $DIRNAME linberry_$MAJOR.$MINOR.orig.tar.gz
