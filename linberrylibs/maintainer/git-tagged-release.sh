...
if [ -z "$1" -o -z "$2" ] ; then
	...
	echo
	echo "This script assumes that a tag called 'linberry-MAJOR.MINOR' already"
	echo "exists.  This script will create two more tags:"
	echo
	echo "    linberry-MAJOR.MINOR.tar.bz2"
	echo "    linberry_MAJOR.MINOR.orig.tar.gz"
	echo
	...
	echo
	echo "Example:  cd linberry-repo"
	echo "          git tag linberry-0.14"
...
git add .
git commit -m "Tarball tree for linberry-$MAJOR.$MINOR.tar.bz2"
git tag -s "linberry-$MAJOR.$MINOR.tar.bz2"
git rm -r debian
git commit -m "Tarball tree for linberry_$MAJOR.$MINOR.orig.tar.gz"
git tag -s "linberry_$MAJOR.$MINOR.orig.tar.gz"

# generate bzip2 tarball
git archive --prefix="$DIRNAME/" "linberry-$MAJOR.$MINOR.tar.bz2" | \
	bzip2 -9c > "maintainer/build/linberry-$MAJOR.$MINOR.tar.bz2"
...
git archive --prefix="$DIRNAME/" "linberry_$MAJOR.$MINOR.orig.tar.gz" | \
	gzip -9c > "maintainer/build/linberry_$MAJOR.$MINOR.orig.tar.gz"
git archive --prefix="$DIRNAME/" "linberry-$MAJOR.$MINOR.tar.bz2" | \
	(cd maintainer/build && tar xf -)
(cd maintainer/build && dpkg-source -b $DIRNAME linberry_$MAJOR.$MINOR.orig.tar.gz)

# finally, generate pristine-tar deltas
pristine-tar -m "Release tarball linberry-$MAJOR.$MINOR.tar.bz2" commit \
	maintainer/build/linberry-$MAJOR.$MINOR.tar.bz2 \
	linberry-$MAJOR.$MINOR.tar.bz2
pristine-tar -m "Release tarball linberry_$MAJOR.$MINOR.orig.tar.gz" commit \
	maintainer/build/linberry_$MAJOR.$MINOR.orig.tar.gz \
	linberry_$MAJOR.$MINOR.orig.tar.gz
