...
void GetChanges(OSyncContext *ctx, LinberryEnvironment *env,
...
static void *initialize(OSyncMember *member, OSyncError **error)
{
	...
	// Create the environment struct, including our Linberry objects
	try {
		env = new LinberryEnvironment(member);
...
static void connect(OSyncContext *ctx)
{
	...
	try {
		...
		LinberryEnvironment *env = (LinberryEnvironment *)osync_context_get_plugin_data(ctx);
...
static osync_bool commit_change(OSyncContext *ctx, OSyncChange *change)
{
	...
	try {
		...
		default:
			...
			osync_debug("linberry-sync", 0, "Unknown change type");
			break;
		}
...
void get_info(OSyncEnv *env)
{
	...
	info->name = "linberry-sync";
	info->longname = "Linberry OpenSync plugin v0.16 for the Blackberry handheld";
