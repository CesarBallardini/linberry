	...
	m_Desc(description)
{
	...
	m_CacheFilename += "/linberry_" + m_Desc + "_cache.txt";
	m_MapFilename += "/linberry_" + m_Desc + "_idmap.txt";
}
...
LinberryEnvironment::LinberryEnvironment(OSyncMember *pm)
	: member(pm),
	m_pin(-1),
	m_DebugMode(false),
	m_password(""),
...
void LinberryEnvironment::ParseConfig(const char *data, int size)
{
...
//void LinberryEnvironment::BuildConfig()
//{
